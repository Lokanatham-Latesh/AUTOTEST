...
class WorkerService:
        ...
        try:
            ...
            if auth_required:
                ...
                if page:
                    ...
                    db.commit()
                    logger.info(f"[TEST_SCENARIO] Page {page_id} marked as is_auth_detected=True")
            await rabbitmq_producer.publish_message(
    ...
    async def process_test_case(self, body: dict):
        ...
        try:
            ...
            if login_test_case:
                logger.info(f"[AUTH] Login test case detected for page {page_id}")
                await self._perform_login_flow(db, login_test_case)

            # Continue normal post-auth discovery
    ...
    def _get_login_test_case(self, db, page_id: int):
        """
        Returns the test case that:
        - requires_auth=True (via scenario)
        - is_valid=True
    ...
    # ----------------- PERFORM LOGIN FLOW ---------------- #
    async def _perform_login_flow(self, db, test_case):
        """
        Performs login using resolved credentials,
        then triggers crawling of landing page.
        """
        ...
        credential_service = TestCredentialService(llm=llm, logger=logger)

        try:
            # Resolve placeholders → actual values
            resolved_data = credential_service.resolve(
                page.id,
                test_case.data.get("test_data", {})
            )
            ...
            if not username or not password:
                logger.warning("[AUTH] Missing credentials for login.")
                return
            ...
            # Detect landing page URL
            landing_url = driver.current_url

            # Save it into test case for tracking
            test_case.expected_outcome["post_auth_landing_url"] = landing_url
            db.commit()

            # Trigger extraction for landing page
            await rabbitmq_producer.publish_message(
                settings.PAGE_EXTRACT_QUEUE,
                {
                    ...
                    "page_id": page.id,
                    "requested_by": test_case.created_by,
                    "timestamp": datetime.utcnow().isoformat()
                },
                priority=6
            )
    ...
    async def _process_post_auth_pages(self, page_id: int, requested_by: int):
        ...
        try:
            page = db.query(Page).filter(Page.id == page_id).first()
            if not page or not page.is_auth_detected:
                return
